...
**인쇄본에서는 전체 테스트 표와 느린 테스트 섹션만 뺍니다.** 전체 테스트 표
120건을 PDF 로 뽑으면 아무도 보지 않습니다. 나머지(요약·도넛, Marker 표,
재시도 표, 실패 카드)는 그대로 담아 비개발자가 인쇄본만 보고도 상황을
파악하게 합니다. 구체적으로:

- 전체 테스트 표 섹션(표 + 필터 버튼)과 느린 테스트 섹션을 통째로 숨김
  (각각 `#all-tests`, `#slow-tests`)
